...
async function validateBetLimits(items, betType, priceData, { userId, sessionId, excludeBetId } = {}) {
    ...
    if (userId && sessionId) {
        try {
            ...
            console.error('Error consultando apuestas existentes para validar límites:', e);
        }
    }
            ...
            if (req.body.confirmLimitOverride === true) {
                ...
                return res.status(400).json({
                    ...
                    exceedData: {
                        ...
                        admissibleLines: admissibleLinesForNumbers(parsed.items, betType, limitCheck.exceedData)
                    }
                });
            }
        } else {
...
app.post('/api/bets/:id/cancel', async (req, res) => {
    ...
    await supabase
        .from('users')
        .update({ usd: newUsd, cup: newCup, bonus_cup: newBonus, bonus_updated_by_admin: null, updated_at: new Date() })
        .eq('telegram_id', userId);
...
app.post('/api/admin/pending-deposits/:id/approve', requireAdmin, async (req, res) => {
    ...
    await supabase
        .from('users')
        .update({ cup: newCup, usd: newUsd, bonus_updated_by_admin: null, updated_at: new Date() })
        .eq('telegram_id', request.user_id);
    ...
    if (isFirstDeposit && newBonus > 0) {
        ...
        await supabase
            .from('users')
            .update({ cup: newCup, bonus_cup: 0, bonus_updated_by_admin: null, updated_at: new Date() })
            .eq('telegram_id', request.user_id);
    }
...
app.get('/api/admin/user/:targetUserId', async (req, res) => {
    const requesterId = parseInt(req.query.userId) || req.verifiedTelegramId;
    ...
    try {
        ...
        if (user.ref_by) {
            const { data: refUser } = await supabase
                ...
                .eq('telegram_id', user.ref_by)
                .maybeSingle();
            if (refUser) {
                refByName = refUser.username ? `@${refUser.username}` : refUser.first_name;
            }
        }

        const hasHadPrincipalBalance = !!(
            parseFloat(user.cup) > 0 ||
            parseFloat(user.usd) > 0 ||
            (bets && bets.length > 0) ||
            (deposits && deposits.some(d => d.status === 'approved')) ||
...
app.get('/api/admin/deleted-users/:telegramId/history', async (req, res) => {
    ...
    const deposits = (typeof data.deposits_history === 'string' ? JSON.parse(data.deposits_history) : (data.deposits_history || []));
    const withdrawals = (typeof data.withdrawals_history === 'string' ? JSON.parse(data.withdrawals_history) : (data.withdrawals_history || []));
    ...
    const processorIds = [...new Set([...deposits, ...withdrawals].map(i => i.processed_by).filter(Boolean))];
    const processors = {};
    if (processorIds.length > 0) {
        const { data: usersData } = await supabase
            .from('users')
            .select('telegram_id, username')
            .in('telegram_id', processorIds);
        if (usersData) for (const u of usersData) processors[u.telegram_id] = u.username || null;
        const missingIds = processorIds.filter(id => !(id in processors));
        if (missingIds.length > 0) {
            const { data: deletedData } = await supabase
                .from('deleted_users')
                .select('telegram_id, username')
                .in('telegram_id', missingIds);
            if (deletedData) for (const u of deletedData) processors[u.telegram_id] = u.username || null;
        }
    }

    res.json({
        deposits: deposits.map(addAmountCup),
        withdrawals: withdrawals.map(addAmountCup),
        processors
    });
});
...
app.get('/api/admin/subadmin-stats/:telegramId', async (req, res) => {
    try {
        ...
        const subadminRoles = await getUserRoles(telegramId);
...
app.post('/api/admin/pending-deposits-role/:id/approve', async (req, res) => {
    ...
    await supabase.from('users').update({ cup: newCup, usd: newUsd, bonus_cup: bonusCup, bonus_updated_by_admin: null, updated_at: new Date() }).eq('telegram_id', request.user_id);
    ...
    if ((action === 'open' && wasOpen) || (action === 'close' && !wasOpen)) {
        ...
        if (wasOpen) {
            ...
            const { data: overrideData } = await supabase
                ...
                .single();
            const currentOverride = overrideData?.value || 'none';
            errorMsg = currentOverride === 'close'
